...
本项目实行 **双版本 / 双分支** 维护策略，每次 Release 均提供两个构建版本供用户按需选择：

| 版本名称 | 分支 | 发布安装包 | 说明与安全性 |
| :--- | :--- | :--- | :--- |
| **纯本地版（推荐）** | `main` | `ExiaInvasion.v*.zip` | **默认安全版**。100% 纯本地运行，源码中彻底剔除所有远程云同步与敏感数据上传逻辑。账号、密码与 Cookie 严格保存在浏览器本地（`chrome.storage.local`），绝对透明安全。 |
| **云端同步版** | `cloud` | `ExiaInvasion.v*-cloud.zip` | **遗留/可选版**。保留了与远程服务器（`backend.nikke-exia.com`）的账号与角色模板云同步功能。仅供有跨设备云同步需求的用户在自担第三方云端风险的前提下使用。 |

> [!TIP]
> 推荐绝大多数用户下载默认的 **纯本地版 (`ExiaInvasion.v*.zip`)**。
